...
NODE_LABELS = [4, 5, 6, 7, 8, 9, 13, 14, 15, 16, 20, 22, 40, 41, 42, 43,
...
PHYSICAL_COORDS: Dict[int, Tuple[float, float]] = {
    4:(0.0,0.355), 5:(0.695,0.355), 6:(0.0,0.15), 7:(0.2,0.0),
    8:(0.5,0.15), 9:(0.695,0.15), 13:(0.0,0.0), 14:(0.2,0.15),
    15:(0.5,0.0), 16:(0.695,0.0), 20:(0.0,-0.15), 22:(0.0,-0.355),
    40:(0.227,0.15), 41:(0.468,0.15), 42:(0.227,0.0), 43:(0.468,0.0),
    23:(0.245,-0.355), 24:(0.395,-0.355), 25:(0.545,-0.355), 26:(0.695,-0.355),
    36:(0.245,-0.385), 37:(0.395,-0.385), 38:(0.545,-0.385), 39:(0.695,-0.385),
    30:(0.227,0.355), 31:(0.468,0.355), 35:(0.695,-0.15),
}

PHYSICAL_NEIGHBORS: Dict[int, List[int]] = {
    4:[30,6], 5:[9,31], 6:[4,13,14,30], 7:[13,42], 8:[9,41], 9:[16,8,31,5],
    13:[6,7,20], 14:[6,40], 15:[16,43], 16:[9,35,15], 20:[23,22,13],
    22:[20,23], 23:[20,22,24,36], 24:[23,25,37], 25:[24,26,35,38],
    26:[25,35,39], 30:[4,31,6], 31:[30,5,9], 35:[16,25,26],
    36:[23], 37:[24], 38:[25], 39:[26], 40:[14], 41:[8], 42:[7], 43:[15],
}
...
def build_state_graph():
    ...
    def find_or_create_slot(ni, theta):
        for k in range(N_LAYERS):
            if node_theta[ni][k] < INF and abs(node_theta[ni][k] - theta) < THETA_THRESH:
                return k
        for k in range(N_LAYERS):
    ...
    for node_idx, phys_id in enumerate(NODE_LABELS):
        ...
        for k in range(N_LAYERS):
            ...
            ax.annotate('', xy=(x+dx, y+dy), xytext=(x, y),
                        arrowprops=dict(arrowstyle='->', color='#E91E63', lw=1.5), zorder=4)
            ax.text(x+dx*1.5, y+dy*1.5, str(si), fontsize=7,
                    ha='center', va='center', color='#4A148C', zorder=6, fontweight='bold')

    for phys_id, (x, y) in PHYSICAL_COORDS.items():
        ax.plot(x, y, 'o', color='#1B5E20', ms=5, zorder=5)
